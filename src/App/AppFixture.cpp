              ...
              if (pixel >= startPosAdjusted && pixel <= endPosAdjusted ) { //if pixel between start and end pos
                ...
                if (indexV != UINT16_MAX) {
                  if (indexV >= leds->nrOfLeds || indexV >= NUM_LEDS_Max) {
                    USER_PRINTF("dev pre [%d] indexV too high %d>=%d or %d (p:%d m:%d) p:%d,%d,%d\n", nrOfLeds, indexV, leds->nrOfLeds, NUM_LEDS_Max, leds->mappingTable.size(), indexP, pixel.x, pixel.y, pixel.z);
                  }
                  else {
